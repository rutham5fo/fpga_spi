...
#set_max_delay -datapath_only -from [get_ports enc_miso] -to [get_pins -of [get_cells -hier in_buffer_reg_reg[0] -filter {name =~ "*axi_spi_interface*/inst*"}] -filter {ref_pin_name == D}] "$enc_in_dly_max"
#set_min_delay -from [get_ports enc_miso] -to [get_pins -of [get_cells -hier in_buffer_reg_reg[0] -filter {name =~ "*axi_spi_interface*/inst*"}] -filter {ref_pin_name == D}] "$enc_in_dly_min"
...
#set_output_delay -clock [get_clocks enc_spiClk_out] 0.0 [get_ports [list "enc_spi_cs_n" "enc_mosi"]]
#set_output_delay -clock [get_clocks enc_spiClk_fwd] -max [expr "$half_unit_interval - $enc_clk_skew + $dec_setup"] [get_ports [list "enc_spi_cs_n" "enc_mosi"]]
#set_output_delay -clock [get_clocks enc_spiClk_fwd] -min [expr "$half_unit_interval + $enc_clk_skew - $dec_hold"] [get_ports [list "enc_spi_cs_n" "enc_mosi"]]

#set_output_delay -clock [get_clocks enc_spiClk_out] -max [expr "$half_unit_interval - $enc_clk_skew + $dec_setup"] [get_ports [list "enc_spi_cs_n" "enc_mosi"]]
#set_output_delay -clock [get_clocks enc_spiClk_out] -min [expr "$half_unit_interval + $enc_clk_skew - $dec_hold"] [get_ports [list "enc_spi_cs_n" "enc_mosi"]]
